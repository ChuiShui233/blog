...
async function fetchMetingPlaylist() {
	...
	try {
		console.log("Fetching Meting API:", apiUrl);
		const res = await fetch(apiUrl);
		if (!res.ok) throw new Error("meting api error");
		const list = await res.json();
		console.log("Meting API Response:", list);
...
:global(.dark) .playlist-panel.float-panel {
...
/* 播放列表项选中状态半透明效果 */
.playlist-item.bg-\[var\(--btn-plain-bg\)\] {
    background: rgba(255, 255, 255, 0.2) !important;
}

.dark .playlist-item.bg-\[var\(--btn-plain-bg\)\] {
...
/* 选中状态的hover效果保持选中样式 */
.playlist-item.bg-\[var\(--btn-plain-bg\)\]:hover {
    background: rgba(255, 255, 255, 0.2) !important;
}

.dark .playlist-item.bg-\[var\(--btn-plain-bg\)\]:hover {
    background: rgba(0, 0, 0, 0.2) !important;
}
</style>
